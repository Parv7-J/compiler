...
impl Parser<'_> {
    pub fn parse_block(&mut self) -> ParseResult<Block> {
        ...
        while !matches!(self.peek(), Some(C_CLOSE),) {
            if matches!(self.peek(), Some(C_OPEN)) {
    ...
    pub fn parse_variant(&mut self) -> ParseResult<Variant> {
        let ident = self.parse_ident()?;
        if self.expect(P_OPEN).is_err() {
    ...
    pub fn parse_field(&mut self) -> ParseResult<Field> {
        let ident = self.parse_ident()?;
        self.expect(P_OPEN)?;
        let ty = self.parse_identty()?;
        self.expect(P_CLOSE)?;
    ...
    pub fn parse_identty(&mut self) -> ParseResult<IdentTy> {
        match self.peek() {
    ...
    fn parse_ptr(&mut self) -> ParseResult<SpannedPtr> {
    ...
    fn parse_arr(&mut self) -> ParseResult<SpannedArr> {
    ...
    pub fn parse_ident(&mut self) -> ParseResult<SpannedIdent> {
        let token = self.expect(TokenKind::Ident)?;
        Ok(SpannedIdent(token.span))
    }

    fn parse_builtintype(&mut self) -> ParseResult<IdentTy> {
    ...
    pub fn parse_ty(&mut self) -> ParseResult<SpannedTy> {
        match self.next() {
            Some(Token {
                kind: TokenKind::Keyword(Keyword::Type(ty)),
                span,
    ...
    pub fn parse_string(&mut self) -> ParseResult<LiteralString> {
        let Token { kind: _, span } = self.expect(TokenKind::String)?;
