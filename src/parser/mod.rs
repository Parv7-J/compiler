...
//
// impl<'a> Parser<'a> {
//     pub fn new(ts: &'a TokenStream<'a>) -> Self {
//         Self {
//             tokens: &ts.0,
//             cursor: 0,
...
//                 Token::String(_) => todo!(),
//                 Token::Ident(_) => todo!(),
//                 Token::Keyword(keyword) => self.parse_keyword(keyword),
//                 Token::Punctuation(punctuation) => todo!(),
//                 Token::Delimiter(delimiter) => todo!(),
//                 Token::Number(_) => todo!(),
//             }
//         }
//
...
//     }
//
//     fn expect(&mut self, token: Token<'_>) -> Result<Token<'_>, String> {
//         let next_token = self.next();
//         if next_token == Some(token) {
//             return Ok(next_token.unwrap());
//         }
//         Err(format!("Expected: {token:?} Found: {next_token:?}"))
//     }
//
...
//
//         self.expect(Token::Delimiter(Delimiter::CurlyOpen))?;
//
//         let mut procs = Vec::new();
//         loop {
//             if self.peek() == Some(Token::Keyword(Keyword::Proc)) {
//                 self.next();
...
//         }
//
//         self.expect(Token::Delimiter(Delimiter::CurlyClose))?;
//         Ok(Require {
//             ident,
//             api,
//             procedures: procs,
...
//
//         if self.peek() == Some(Token::Keyword(Keyword::Also)) {
//             self.next();
//
//             loop {
//                 let api = self.parse_ident()?;
...
//         let ident = self.parse_ident()?;
//
//         self.expect(Token::Delimiter(Delimiter::SquareOpen))?;
//
//         let mut args = Vec::new();
//         if self.peek() != Some(Token::Delimiter(Delimiter::SquareClose)) {
//             loop {
...
//         }
//
//         self.expect(Token::Delimiter(Delimiter::SquareClose))?;
//
//         let mut return_value = None;
//         if self.peek() == Some(Token::Punctuation(Punctuation::Colon)) {
//             self.next();
...
//         match self.next() {
//             Some(Token::Ident(i)) => Ok(Ident(self.ident_to_id(i))),
...
//
//         if self.peek() == Some(Token::Delimiter(Delimiter::CurlyClose)) {
//             self.next();
...
//         let ident = self.parse_ident()?;
//
//         if self.peek() != Some(Token::Delimiter(Delimiter::ParenOpen)) {
//             return Ok(Variant::Ident(ident));
//         }
//         self.next();
//
//         let ty = match self.next() {
//             Some(Token::Keyword(Keyword::Type(t))) => IdentTy::Type(t),
//             Some(Token::Ident(i)) => IdentTy::Ident(Ident(self.ident_to_id(i))),
//             token => return Err(format!("Expected Type/Ident, Found: {token:?}")),
//         };
//
//         self.expect(Token::Delimiter(Delimiter::ParenClose))?;
//
...
//         match self.next() {
//             Some(Token::Keyword(Keyword::Type(t))) => Ok(IdentTy::Type(t)),
//             Some(Token::Ident(i)) => Ok(IdentTy::Ident(Ident(self.ident_to_id(i)))),
...
//         let ident = self.parse_ident()?;
//
//         self.expect(Token::Delimiter(Delimiter::ParenOpen))?;
//
//         let ty = match self.next() {
//             Some(Token::Keyword(Keyword::Type(t))) => IdentTy::Type(t),
//             Some(Token::Ident(i)) => IdentTy::Ident(Ident(self.ident_to_id(i))),
...
//     }
//
//     fn ident_to_id(&mut self, ident: &'a str) -> usize {
